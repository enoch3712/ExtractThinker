...
| [#48](https://github.com/enoch3712/ExtractThinker/issues/48) | Events: Add IDP events  | Implemented; delivery pending | Vision page signal detector, handwritten/chart/image events, local rules and callbacks; pipeline and failure-order regressions |
...
- DocumentLoaderEvents supports handwritten/chart/image detection via a configured vision LLM, custom detectors and local rules. Detection finishes before callbacks run.
- GitHub confirms #373 merged (25 issues closed; four remaining).
- 224 offline tests passed on Python 3.12; nine event tests passed on Python 3.9 and 3.13. Strict docs build passed.
