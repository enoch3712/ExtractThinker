...
class DocumentLoaderAzureForm(CachedDocumentLoader):
    ...
    def load_content_from_file(self, file_path: str) -> Union[str, object]:
        try:
            with open(file_path, "rb") as document:
                result = self.client.analyze_document("prebuilt-layout", document)
    ...
    def load_content_from_stream(self, stream: Union[BytesIO, str]) -> Union[str, object]:
        try:
            result = self.client.analyze_document("prebuilt-layout", stream)
            return self.process_result(result)
        except Exception as e:
    ...
    def process_result(self, result):
        ...
        for page in result.pages:
            paragraphs = [p.content for p in page.paragraphs]
            tables = self.build_tables(page.tables)
    ...
    def build_points(self, bounding_box):
        return [{"x": point.x, "y": point.y} for point in bounding_box]

# Definition for cachedmethod decorator, assuming CachedDocumentLoader is properly implemented
